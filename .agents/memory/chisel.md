...
## 2025-02-28 - Chisel's Movement Splitting Refactoring

Learning: When extracting logic from Numba JIT compiled monolithic files like `movement.py` into a package structure, be extremely careful about import circularities and where the `math.py` file sits in relation to `core.py`. Additionally, the fallback Python functions (like `_choose_neighbour_by_flow_probability_python`) frequently depend on Numba variables that might seem isolated, but are necessary for tests. Ensuring they remain in the correct public import path is essential so that tests running mock assertions still find the functions.

Action: Always run `uv run pytest tests/unit/engine/systems/` immediately after splitting a module to verify that test mocks haven't been broken. Also ensure `__all__` is sorted correctly using `ruff format/check`.
