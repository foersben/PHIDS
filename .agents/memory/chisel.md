...
## 2026-08-06 - Discovered Advanced Scaling Techniques
Learning: While auditing the documentation against the codebase, I discovered that the engine implementation contains undocumented performance techniques: bypassing the ECS query generator overhead using direct private dictionary access (`world._component_index` and `world._entities`), leveraging bitwise modulo (`width - 1`) for toroidal wrap on power-of-2 grids, and utilizing pre-computed numpy arrays for O(1) stationary collision detection (`_is_swarm_anchored`).
Action: I should recognize and leverage these techniques (like direct dict access with strict lifecycle guarantees, or bitwise modulo for toroidal wraps) when optimizing other computational hot-paths in the future.
