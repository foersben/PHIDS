...
## 2026-08-13 - [Trigger Chain Invariants] Learning: Premature Activation Condition Evaluation Blocked Substance Synthesis Action: The PHIDS engine defines a sequence for plant chemical defenses: (1) Trigger initiated (e.g., herbivore attack), (2) Synthesis (takes time), (3) Release/Activation (requires activation_condition to be met). However, the engine's `_process_single_trigger` routine was prematurely evaluating the `activation_condition` (intended to gate emission) alongside the `initiator` (intended to gate synthesis). This meant that if the activation condition (like "requires another substance to be active") was not yet met, the plant would completely fail to begin synthesizing the defense compound, permanently breaking alarm-chain responses and compound triggers.

Tests failed to catch this due to several gaps:
1.  **Missing Combinatorial Tests:** Tests verified synthesis logic (e.g., `test_trigger_synthesis_rearm.py`) but always initialized `trig.activation_condition_dump = None`. The test suite contained NO tests evaluating a trigger that had BOTH an initiator and an activation condition.
2.  **State Verification Gaps:** Existing tests largely focused on component existence or basic attribute checking but missed the sequential invariant mapping where an action traverses state boundaries (Inactive -> Synthesizing -> Active).
3.  **Behavioral Invariant Drift:** The architectural intent (synthesis precedes activation) drifted from the codebase implementation, where one monolithic `_process_single_trigger` handled immediate actions (Resource Withdrawal) identically to deferred ones (Synthesize Substance).

**Action:** [How to apply next time] Always construct comprehensive invariant matrices across temporal boundaries (e.g., tick sequences) when dealing with deferred effects. Enforce test coverage that tests the combinatorial intersection of all trigger schema properties, rather than isolated happy paths. Furthermore, differentiate immediate versus deferred execution actions early in the engine logic to prevent applying gating logic (like `activation_condition`) to the wrong temporal phase.
