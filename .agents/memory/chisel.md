...
## 2025-02-19 - Extracted movement.py Monolith into a Package
Learning: Numba JIT kernels and branchless capacity masking logic in `movement.py` reached >800 lines, blending core swarm mechanics with math utility functions. Splitting monolithic logic safely requires 1:1 functional structural copying to avoid breaking JIT determinism, and routing all external repository imports directly through an `__init__.py` file keeps backwards compatibility intact while flattening the package namespace.
Action: When extracting deeply nested mathematical engines, explicitly construct a package `__init__.py` with `from module import function as function` block to re-export the exact public API. This eliminates the need to update `getattr` reflection boundaries across ECS unit tests.
