...
## 2025-09-15 - Fast API Router Extraction and Fast API State Side-Effects
Learning: When refactoring monolithic FastAPI routers into sub-packages, directly extracting helper functions that implicitly mutate application state (like modifying the `draft` state in memory via `get_draft`) creates hidden coupling. While `router.include_router()` safely recomposes the routing surface, the helper functions must perfectly replicate their previous side effects. Furthermore, `ruff` checks are highly sensitive to copy-pasted `from __future__ import annotations` and docstring headers, often requiring multi-pass format and import sorting via `--fix --unsafe-fixes`.
Action: Next time a router is split, strictly run `uv run ruff check .` before test execution to ensure all new sub-modules are perfectly compliant. Pay special attention to duplicated `TYPE_CHECKING` blocks and unused imports that occur when breaking up monolithic execution flows.
