...
## 2026-07-21 - [Docs] Learning: Technical documentation can easily drift into hallucinated features (zero-allocation arrays, wrong loop execution steps). Action: Always verify architectural claims in markdown against the actual codebase implementation (`ECSWorld` and `SimulationLoop` classes) before accepting documentation as ground truth.
