...
## 2025-02-27 - ECS Multi-Component Query Optimization
**Learning:** For multi-component queries in `ECSWorld.query`, iterating over sets with generator expressions containing `all(...)` introduces significant pure-Python looping overhead. Utilizing Python`s built-in `set.intersection_update()` operates at the C-API level and is substantially faster. Sorting the query sets by length and sequentially intersecting against a copy of the smallest set yielded roughly a 4.7x speedup in isolated benchmarks and reduced tick overhead.
**Action:** Always favor native Python set operations (`intersection_update`, `difference_update`) for mathematical set processing in performance-critical code over looping generator comprehensions.
