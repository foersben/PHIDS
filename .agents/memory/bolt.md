...
## 2024-05-18 - [Avoid np.any allocation in signaling boolean checks]

**Learning:** Using `np.any(layer >= val)` silently allocates a large temporary boolean array (shape WxH). In tight simulation loops, this memory allocation causes significant overhead.
**Action:** Replaced `np.any(layer >= val)` with `layer.max() >= val` which operates directly on the array and returns a scalar without allocating a boolean mask. This speeds up the deterministic hot path.
