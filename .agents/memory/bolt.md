...
## 2026-08-15 - [Avoid Boolean Array and Sum Allocations in Numpy (Redux)]

**Learning:** During profiling of the double-buffering execution in the ECS simulation engine, I discovered that basic Numpy checks like `np.any(layer >= SIGNAL_EPSILON)` silently allocate large temporary boolean arrays of shape `(width, height)`. This had already been fixed in one part of `GridEnvironment.diffuse_signals`, but I found another instance where `np.any` was being used right after diffusion (`if not np.any(self._signal_layers_write[s] >= SIGNAL_EPSILON):`).
**Action:** Replaced `np.any(layer >= val)` with `float(np.amax(layer)) >= val`. Using `np.amax` (with a cast to `float` to avoid scalar typing issues, although `np.max` works as well) evaluates the maximum directly on the array and returns a scalar without allocating a boolean mask. This is fundamentally necessary to avoid dynamic allocation during the deterministic hot path, resulting in roughly a 6-7% increase in throughput (from 359 ticks/s to 383 ticks/s on dense tests).
