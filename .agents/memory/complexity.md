...
* **Target Function:** src/phids/api/routers/config/trigger_rules.py and _build_node_updates
* **Selection Rationale:** Selected due to a complexity score of 15 (exactly on threshold). It is a pure API layer data transformation function consisting of independent `if/elif` branches. Easy to untangle by extracting branches into separate helper functions with 0 performance risk since it is completely outside the engine's hot paths.
* **Before/After Score:** 15 vs. 5
* **Performance Assessment:** Common sense confirms zero performance regression as this is an API configuration route with no complex iterations or mathematical operations, running outside the core simulation loop.
* **Test Verification:** Confirmed that all linting, unit tests, and complexity checks pass.
