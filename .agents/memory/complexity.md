...
* **Selection Rationale:** Selected due to a high cognitive complexity score of 18, and clear untangling path. It was highly suitable as the function consisted of multiple independent export blocks (`csv`, `tex_table`, `tex_tikz`, `png`) that could be safely extracted into separate private helper functions without degrading engine performance (it runs completely decoupled from the simulation hot-loop).
* **Before/After Score:** 18 vs. well below threshold (does not appear in complexipy output).
* **Performance Assessment:** Common-sense reasoning confirms zero performance regression: extracting formatting conditionals into their own private module-level functions in an API orchestration tool has virtually zero overhead, and the function operates entirely outside of any hot path or engine execution loop.
* **Test Verification:** Confirmed that all linting, unit tests, and complexity checks pass.
