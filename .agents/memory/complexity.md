...
* **Selection Rationale:** Selected this function because it was highly decoupled API-layer code handing multiple text-based file exports (CSV, PNG, TiKZ, LaTeX) using a monolithic `if/elif` structure. Extracting each format into modular private module functions cleanly isolated the logical pathways. This carried zero performance risk since the API is completely untethered from the `@njit` JIT simulation engine loops.
* **Before/After Score:** 18 vs. 3
* **Performance Assessment:** Common-sense reasoning confirms zero performance regression: logic modifications solely replaced large conditionally evaluated blocks with identical, deferred invocations to independent functions, which run instantly on slow API HTTP calls.
* **Test Verification:** Confirmed that all linting, unit tests, and complexity checks pass.
