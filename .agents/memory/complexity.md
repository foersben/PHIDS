...
* **Target Function:** `src/phids/api/routers/config/trigger_rules.py` and its `_build_node_updates` function
* **Selection Rationale:** The `_build_node_updates` function had a peak complexity score of 15 (tied for highest codebase wide). Extracting the branch cases out to small helper subroutines reduces the complexity completely while carrying practically zero performance overhead on this non-hot-path API endpoint.
* **Before/After Score:** 15 vs. 5 (with the new helpers scoring 2 or less)
* **Performance Assessment:** The function operates as a basic standard API config layer setting parameter dictionaries with purely O(1) branching. The extracted methods introduce only minute function call overheads strictly off the JIT engine hot paths. No benchmarks needed, guaranteed zero performance regression.
* **Test Verification:** Confirmed that all linting, unit tests, and complexity checks pass.
