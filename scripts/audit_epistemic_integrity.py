...
class EpistemicASTVisitor(ast.NodeVisitor):
    ...
    def visit_If(self, node: ast.If) -> None:
        """Check for scalar if/else state branching inside JIT kernels."""
        if self._in_jit:
            ...
            self.findings.append(
                Finding(
                    ...
                    snippet=snippet,
                    description=f"Branching conditional 'if' inside JIT function '{self._current_function}'.",
    ...
    def visit_Call(self, node: ast.Call) -> None:
        ...
        if isinstance(node.func, ast.Attribute) and node.func.attr == "floor":
            if "feeding" in self.rel_path or "population" in self.rel_path or "metabolism" in self.rel_path:
                snippet = self._get_line_snippet(node.lineno)
                ...
                self.findings.append(
                    Finding(
                        ...
                        suggested_remediation=(
                            "Accumulate fractional attrition into a continuous residual pool or apply "
                            "stochastic rounding."
                        ),
                    )
                )
        ...
        if "src/phids/engine/systems" in self.rel_path:
            for val in ast.walk(node.value):
                if isinstance(val, ast.Constant) and isinstance(val.value, (int, float)):
                    num = val.value
                    if num not in NEUTRAL_LITERALS:
                        snippet = self._get_line_snippet(node.lineno)
                        slice_id = (
                            5
...
def check_specialized_slice_invariants(root_path: Path) -> list[Finding]:
    ...
    # Slice 1: Check grid coordinate power-of-two assertions
    ...
    # Slice 5: Check feeding repulsion timer magic number
    ...
    # Slice 9: Check Dual-Proxy placeholder growth rate in constants.py
...
def format_markdown_report(findings: list[Finding], target_slice: int | None = None) -> str:
    ...
    for s_id in range(1, 11):
        ...
        lines.append(
            f"| {s_id} | {SLICE_METADATA[s_id]['name']} | {s_crit} | {s_maj} | {s_min} | {s_inf} | {len(s_findings)} |"
        )

    lines.extend(["", "## Detailed Slice Findings", ""])
