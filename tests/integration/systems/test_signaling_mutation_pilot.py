...
def test_environmental_signal_initiator_hill_kinetics(
    add_plant: Callable[..., int],
) -> None:
    """An environmental signal trigger using hill kinetics activates when priming factor meets activation threshold."""
    ...
    # Set ambient signal concentration equal to half_saturation (Kd = 0.05), priming factor = 0.5 >= 0.05
    env.signal_layers[0, 1, 1] = 0.05

    trigger = TriggerConditionSchema(
        initiator=EnvironmentalSignalInitiator(
            signal_id=0,
            response_curve="hill",
            half_saturation=0.05,
            hill_cooperativity=2.0,
        ),
        action=SynthesizeSubstanceAction(
            substance_id=1,
            synthesis_duration=1,
