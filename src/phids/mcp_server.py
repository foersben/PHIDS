...
def export_telemetry_data(
    ...
    try:
        ...
        if format == "csv":
            data = _export_csv(filtered_rows, normalized_data_type, tick_interval, columns)
            return {"status": "success", "format": format, "data": data}

        elif format == "tex_table":
            data = _export_tex_table(rows, columns, flora_ids, herbivore_ids, tick_interval)
            return {"status": "success", "format": format, "data": data}

        elif format == "tex_tikz":
            data = _export_tex_tikz(
                filtered_rows,
                normalized_data_type,
                flora_names,
                herbivore_names,
