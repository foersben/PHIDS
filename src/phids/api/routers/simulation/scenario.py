...
"""Simulation scenario routes.

This module provides routes for importing, exporting, and loading
scenarios into the simulation draft state.
"""
...
@router.post("/api/scenario/load", summary="Load config directly into simulation loop")
async def load_scenario(config: SimulationConfig) -> dict[str, int | str]:
    ...
    Args:
        config: Strict validation schema representing the entire simulation setup.

    Returns:
        JSON confirmation payload echoing the newly bound grid dimensions.
    """
    if api_main._sim_task is not None and not api_main._sim_task.done():
        api_main.logger.info("Cancelling existing background simulation task before loading scenario")
    ...
    api_main.logger.info(
        "Scenario loaded: %dx%d grid, %d flora, %d herbivores",
        config.grid_width,
        config.grid_height,
