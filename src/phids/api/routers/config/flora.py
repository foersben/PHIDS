...
def _build_flora_updates(
    ...
    passive_mech_dmg: float | None,
    passive_digest_mod: float | None,
    current_passive_defenses: object,
) -> dict[str, object]:
    ...
    if passive_mech_dmg is not None:
        passive_updates["mechanical_damage_per_bite"] = max(0.0, passive_mech_dmg)
    if passive_digest_mod is not None:
        passive_updates["digestibility_modifier"] = max(0.0, min(1.0, passive_digest_mod))
    if passive_updates and hasattr(current_passive_defenses, "model_copy"):
        updates["passive_defenses"] = current_passive_defenses.model_copy(update=passive_updates)
    return updates
