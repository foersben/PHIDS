...
def _build_herbivore_updates(
    ...
    """Build update fields for a herbivore_presence node."""
...
def _build_substance_updates(substance_id: int | None) -> dict[str, object]:
    """Build update fields for a substance_active node."""
    updates: dict[str, object] = {}
...
def _build_signal_updates(signal_id: int | None, min_concentration: float | None) -> dict[str, object]:
    """Build update fields for an environmental_signal node."""
    updates: dict[str, object] = {}
...
def _build_node_updates(
    ...
    if current_kind == "environmental_signal":
        return _build_signal_updates(signal_id, min_concentration)
