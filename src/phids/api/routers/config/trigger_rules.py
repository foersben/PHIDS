...
def _update_environmental_signal(signal_id: int | None, min_concentration: float | None) -> dict[str, object]:
    updates: dict[str, object] = {}
...
def _build_node_updates(
    ...
    if current_kind == "herbivore_presence":
        return _update_herbivore_presence(herbivore_species_id, min_herbivore_population)

    if current_kind == "substance_active":
        return {"substance_id": substance_id} if substance_id is not None else {}

    if current_kind == "environmental_signal":
        return _update_environmental_signal(signal_id, min_concentration)
