...
def _build_herbivore_updates(
    species_id: int | None, min_pop: int | None
) -> dict[str, object]:
    updates: dict[str, object] = {}
    if species_id is not None:
        updates["herbivore_species_id"] = species_id
    if min_pop is not None:
        updates["min_herbivore_population"] = max(1, min_pop)
    return updates


def _build_environmental_updates(
    signal_id: int | None, min_concentration: float | None
) -> dict[str, object]:
    updates: dict[str, object] = {}
...
def _build_node_updates(
    ...
    if current_kind == "environmental_signal":
        return _build_environmental_updates(signal_id, min_concentration)
