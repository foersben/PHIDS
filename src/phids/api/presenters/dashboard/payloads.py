...
def _build_flora_energy_payload(
    ...
    for species in config.flora_species:
...
def build_live_dashboard_payload(
    ...
    *,
    ...
    plants = _build_plants_payload(world, env, flora_names, owned_substances)
    swarms = _build_swarms_payload(world, env, herbivore_names)

    live_flora_species_ids = {
        sid for sid in (_coerce_int(species_id, default=-1) for species_id in plants["species_id"]) if sid >= 0
    }
    all_flora_species, species_energy = _build_flora_energy_payload(loop.config, env, live_flora_species_ids)
