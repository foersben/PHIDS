"""Dashboard presenter: ECS-to-JSON serialisation for live grid payloads.

This module implements the core payload assembly layer that bridges the PHIDS simulation runtime —
represented by a live :class:`~phids.engine.loop.SimulationLoop` — and the structured JSON
dictionaries consumed by the browser canvas renderer.

Architectural Rationale
-----------------------
In the original architecture, UI serialisation functions resided in ``phids.api.main`` alongside HTTP wiring.
This co-location violated the single-responsibility principle. Relocating these functions to a dedicated
presenter package restores a clean boundary. This specific module houses the massive root dashboard payload
builder, while tooltip, substance, and utility logic have been factored into sibling modules within
``phids.api.presenters``.

Dependency Injection
--------------------
All public functions accept ``substance_names: dict[int, str]`` as a keyword-only argument.
This makes the functions deterministically testable with arbitrary name dictionaries and eliminates
hidden coupling to mutable application state.
"""
...
from phids.api.presenters.mycorrhiza import _build_live_mycorrhizal_links
from phids.api.presenters.substances import _is_live_substance_visible
from phids.api.presenters.utils import _coerce_int
