...
def get_grid_masks(width: int, height: int) -> tuple[bool, int, int]:
    ...
    where mask_x = width - 1 and mask_y = height - 1. When either dimension is not a power
    of 2, is_pow2 is False and mask_x/mask_y are set to -1 (indicating bitwise masking cannot
    be used and modulo operations must be used instead).
    ...
    mask_x = (width - 1) if is_pow2 else -1
    mask_y = (height - 1) if is_pow2 else -1
