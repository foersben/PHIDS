...
### HTMX State Synchronization and Out-of-Band (OOB) Swaps

The Simulation Control Panel (Play/Pause/Resume) leverages HTMX for zero-refresh UI interactions. Previously, the UI attempted to sync button state (e.g., swapping `hx-post` from `/api/simulation/start` to `/api/simulation/pause`) using client-side JavaScript (`htmx.process(btn)`). However, dynamically mutating and re-processing HTMX control attributes within polling loops causes an exponential explosion of duplicate event listeners, eventually locking the browser UI thread (DDOSing the browser tab).

To guarantee robust, zero-client-logic synchronization, PHIDS uses **HTMX Out-of-Band (OOB) Swaps**. When the server renders the `#sim-status` badge via `/api/ui/status-badge`, the FastAPI presenter also includes the fully rendered HTML for the `#sim-main-action-btn` tagged with `hx-swap-oob="true"`. HTMX atomically swaps both elements directly from the server response. This ensures the UI button state perfectly matches the backend physics engine state without any fragile JavaScript race conditions or duplicate listeners.

Furthermore, to guarantee immediate visual feedback during **delayed pauses** (when the server must finish executing a heavy Numba calculation tick before yielding), the button HTML embeds a `.htmx-indicator` SVG spinner. HTMX automatically appends the `.htmx-request` class to the button during the network round-trip. CSS handles the visual fade-out of text and fade-in of the spinner, providing instantaneous user feedback while the OOB swap prepares the final "Resume" state.
