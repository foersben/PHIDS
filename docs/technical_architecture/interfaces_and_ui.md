...
This header cleanly instructs the client-side HTMX instance to fire these events globally. The button itself listens to `updateMainActionBtn from:body` and triggers a subsequent `GET /api/ui/main-action-btn` with `hx-swap="outerHTML"`, completely replacing itself with the fresh, server-rendered "Pause" button state. This guarantees the UI button state perfectly matches the backend physics engine state without fragile JavaScript race conditions, duplicate listeners, or OOB-related DOM corruption.

Furthermore, by placing the `hx-get` and `hx-swap="outerHTML"` directly on the button rather than a wrapper container, the UI naturally sidesteps CSS cascade bugs related to `.htmx-request` classes, ensuring that the `.htmx-indicator` SVG spinner strictly appears only during active network round-trips and is never stuck in a frozen "loading" phase.
