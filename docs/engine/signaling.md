...
For signals, emission is constrained by a fixed per-tick mass budget (`SUBSTANCE_EMIT_RATE`) that is partitioned across one airborne slot at the owner coordinate and all valid mycorrhizal relay targets. If a source has `k` valid relay neighbors, each slot receives `SUBSTANCE_EMIT_RATE / (1 + k)`. This conservation rule prevents root-network fan-out from cloning mass and keeps source strength comparable across network topologies. Relay writes are therefore budget shares, not additional generation terms.
...
with source term $Q_s^t$ generated by active emitters and relay deposits. Runtime diffusion uses preallocated write buffers and swaps visibility only after each pass. Concentration tails below `SIGNAL_EPSILON` are truncated to suppress subnormal floating-point overhead and preserve stable sparse-field behavior. Emission at the owner coordinate is intentionally uncapped by a hard `min(1.0, ...)` clamp, allowing concentration to approach a physically meaningful steady state where source injection balances diffusion and decay rather than producing an artificial plateau front.
