...
For airborne signals, the per-tick emission budget is partitioned between local air concentration and mycorrhizal relay targets so that one active substance contributes a fixed source term per tick rather than duplicating mass along each root edge. Airborne signal concentrations are not hard-capped at unity; under sustained forcing they approach a dynamic steady state determined by emission, diffusion, and decay.
