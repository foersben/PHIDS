...
Instead, the UI edits a server-side draft object defined in `src/phids/api/ui_state.py`.
...
`DraftState` is a mutable dataclass that accumulates the full scenario-builder state. It owns:
...
```mermaid
flowchart TD
	A[Operator edits builder controls] --> B[Route mutates DraftState]
...
+
+`DraftState` is more than a bag of form values. It contains a large amount of structural logic.
+
+### Species and matrix maintenance
+
+When flora or predator species are added or removed, the draft performs coordinated maintenance of:
+
...
+- initial placements.
+
+This is crucial because the UI is effectively editing a bounded scientific model with coupled
+indices, not merely a set of unrelated form rows.
+
