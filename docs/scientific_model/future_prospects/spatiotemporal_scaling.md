...
status: active
version: 1.4
description: "A unified roadmap for scaling PHIDS to large physical biomes, addressing memory limits, subloop computability, storage constraints, kinetic abstraction, and edge-case scaling horizons with precise computability metrics."
tags:
- phids
...
To simulate an entire physical biome (e.g., a 1 km² mixed forest) realistically, PHIDS cannot rely on arbitrary grid units and abstract ticks. The system must rigorously enforce Dimensional Anchoring, strictly coupling physical space, thermodynamic energy, and biological time into a unified computational framework.

This roadmap unifies the empirical database pipeline (ETL), the Entity-Component-System (ECS), telemetry storage, and the multi-stage simulation loop to successfully model large-scale ecosystems without violating hardware limits, branching efficiency, or visual interpretability.

## 1. The Spatiotemporal Dimensional Anchor & Memory Limits

Before a forest scenario is initialized, the `SimulationConfig` must explicitly define the base physical units ($\Delta L$, $\Delta \tau$, $\Delta E$). Everything else in the engine scales relative to these constants.
...
## 2. Multi-Scale Temporal Decoupling (The "Modulo-Gated" Loop)
...
To resolve this, the `SimulationLoop.step()` implements **Modulo-Gating**, executing distinct biological systems at different natural frequencies:
...
* **The Slow Loop (Every 168 Ticks / Weekly):**
  * *Growth & Demographics:* Plants apply their accumulated photosynthetic growth. Swarms with massive caloric surpluses execute mitosis. Dead entities are culled from the Spatial Hash.
...
## 3. Behavioral Abstraction: Stochastic Von Neumann Kinematics
...
* **The Von Neumann Neighborhood:** A swarm does not evaluate a 360-degree continuous radius (Moore 8-way). It only looks at its four adjacent orthogonal tiles: North, South, East, and West ($N, S, E, W$). This reduces DRAM fetches by $50\%$ compared to Moore neighborhoods.
...
## 4. Visual Representation: The "Temporal Lens" (Zarr Replay)
...
To resolve this, the UI completely bypasses the simulation engine and reads historical data directly from the Zarr replay buffers using a **Temporal Lens Toggle**:

* **Micro Lens (Sparse Zarr Reads):** The UI fetches only the sparse swarm coordinate chunks from the Zarr store for every tick. For $1000$ swarms, this is $12 \text{ KB}$ per tick. At 60 TPS, this uses only $720 \text{ KB/s}$ HTTP fetch bandwidth, making real-time foraging visually perfectly fluid and web-viable.
...
## 5. Storage, Replay, and Telemetry (Zarr + Polars)
...
* **Storage Strategy:** These are appended every tick as flat dictionaries and lazily materialized into highly compressed Polars DataFrames. The append operation takes amortized $O(1)$ time ($\sim 5 \mu\text{s}$).

### B. Zarr (High-Density Spatial Replay)

* **Storage Strategy:** Zarr chunks the $1024 \times 1024$ matrices into $256 \times 256$ blocks ($256 \text{ KB}$ each, aligning with L2 cache sizes). Blosc LZ4 compression achieves $> 2 \text{ GB/s}$ compression throughput. Compressing the active VOC layers takes $< 2 \text{ ms}$.
...
## 6. Allometric Scaling & Population Densities

To maintain realistic flora and fauna densities on a $1 \text{ m}^2$ resolution grid, the ETL pipeline enforces strict allometric scaling laws derived from PanTHERIA and TRY.
...
## 7. Unresolved Scaling Horizons: Edge Effects & Spatial Congestion

As PHIDS pushes toward full $1 \text{ km}^2$ macro-scale simulation, three specific spatial scaling issues are rigorously addressed:
...
* **The Fix (Toroidal Grid):** The grid topology transitions to a Torus (wrap-around). Numba spatial convolution kernels use `boundary='wrap'`, and coordinate updates utilize single-cycle bitwise masking (`x & (width-1)`) to seamlessly simulate an infinite, continuous forest.

### B. Volumetric Collision (Swarm Stacking)

Because 1 cell equals exactly $1 \text{ m}^2$, the ECS Spatial Hash faces infinite entity stacking.

* **The Fix (Branchless Capacity Masking):** The spatial hash implements a rigid `max_biomass_per_cell` mask. Preventing swarm stacking could induce branching (`if tile > capacity`), but instead we evaluate capacity as a boolean mask `(tile_biomass < max_cap)`. We multiply the Softmax probability vector by this mask. If the tile is full, the probability instantly becomes `0.0`. This avoids CPU pipeline flushes associated with branch mispredictions in the Numba loop.

### C. The Seed Dispersal Convolution Bottleneck

Currently, reproducing plants calculate an anemochorous kernel matrix via spatial convolution sum. Standard spatial convolution for reproduction is $O(N_{\text{plants}} \times r^2)$. For $10,000$ plants dropping seeds in a 50-cell radius, this requires $25,000,000$ floating-point operations.

* **The Fix (O(1) Stochastic Raycasting):** Macroscopic seed dispersal transitions to Discrete Stochastic Raycasting. Generating a vector angle and magnitude using NumPy's `PCG64` generator takes $\sim 2 \text{ ns}$ per plant. The computational complexity drops to $O(N_{\text{plants}})$, reducing the dispersal phase from $\sim 15 \text{ ms}$ (convolution) to $\sim 20 \mu\text{s}$ (raycasting)-a $750\times$ sub-system speedup during the Slow Loop.
