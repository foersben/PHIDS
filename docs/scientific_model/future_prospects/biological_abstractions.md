...
- id: simulation_control
  resource: src/phids/api/routers/simulation/control.py
- id: simulation_scenario
  resource: src/phids/api/routers/simulation/scenario.py
- id: zarr_replay
...
!!! note "Understanding the Data-Flow Matrix (Conceptual Metaphor)"
    Think of the table below as a high-level biological ledger showing how plants balance daily energy and physical body size.

    In the real world, plants manage two distinct biological assets: **caloric energy** (mobile sugars generated by sunlight for daily metabolism) and **structural biomass** (dense, woody physical matter providing structural integrity). A plant can be starved of sugar while remaining physically robust, or conversely, have abundant sugar while still being a fragile seedling.

    * **Event Column:** Major life events including photosynthesis, growth, herbivore grazing, symbiotic payments, trampling, and starvation.
    * **Precondition Column:** The physical and caloric requirements necessary for that event to occur.
    * **State Transformation:** The exact mathematical transfer between energy and physical biomass.
    * **ECS Resolution:** The engine loop phase responsible for calculating the update.

### Scenario Dynamics in Words (Technical Overview)

This matrix specifies the discrete state-transition rules governing the dual-proxy partition between caloric energy ($E_{\text{current}}$) and physical structural biomass ($M_{\text{structural}}$):

1. **Autotrophic Photosynthesis:** When sunlight illuminates foliage, solar energy converts into mobile caloric pool reserves up to the species saturation ceiling $E_{\text{max}}$. Structural biomass remains untouched during continuous diurnal updates.
2. **Biomass Synthesis & Conversion:** Once surplus caloric reserves exceed the activation threshold $\text{Cost}_{\text{growth}}$, metabolic systems convert mobile carbohydrates into permanent lignified mass ($\Delta M = +\delta M$). This structural investment permanently increases physical resilience against herbivore trampling.
3. **Grazing & Herbivory:** Heterotrophic feeding extracts mobile energy from foliar tissue. If herbivore consumption outpaces photosynthate accumulation, caloric reserves fall toward zero while physical woody architecture temporarily remains intact.
4. **Symbiotic Network Upkeep:** Maintaining mycorrhizal fungal conduits requires continuous carbon payments. If caloric energy drops below the upkeep threshold ($E_{\text{current}} < \text{Upkeep}$), the connection mask drops branchlessly via SIMD float multiplication (`E > Upkeep`).
5. **Physical Trampling & Starvation:** Fragile seedlings below adult structural thresholds ($M_{\text{structural}} < M_{\text{adult}}$) suffer mechanical destruction proportional to swarm herd biomass. When caloric reserves reach zero, starvation initiates spatial cell deallocation.
