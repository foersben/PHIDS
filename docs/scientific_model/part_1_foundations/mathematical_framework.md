...
| :--- | :--- | :--- | :--- |
| **Toroidal Grid Geometry (Periodic Boundaries)** | Unbounded continuous world topology preserving mass conservation and wind transport without edge-effect boundary artifacts. | Dual-dispatch bitwise wrapping `(x + dx) & (width - 1)` for power-of-two grids with general modulo fallback across Numba `@njit` kernels and ECS spatial hash keying. | [Reaction-Diffusion PDEs](../part_3_signaling_and_transport/reaction_diffusion.md) |
| **Volatile Signal Dispersion & Wind Advection** | Airborne Volatile Organic Compound (VOC) warnings spread downwind from damaged plants to prime neighbors across toroidal boundaries. | 2D Semi-Lagrangian advection + $3\times 3$ isotropic Gaussian convolution stencil + subnormal float truncation ($< \text{SIGNAL\_EPSILON} = 1\times 10^{-4} \to 0.0$). | [Reaction-Diffusion PDEs](../part_3_signaling_and_transport/reaction_diffusion.md) |
...
| **Constitutive Morphological Defenses** | Mechanical thorns inflict physical mouthpart trauma; cell-wall lignin/silica reduces caloric digestibility. | Poisson-equivalent continuous stochastic damage accumulation ($\lfloor D_{\text{mech}} \rfloor + \mathbb{I}(U < \text{frac})$) and caloric discount factor $\eta_{\text{net}}$ in `feeding.py`. | [Constitutive Morphological Defenses](../part_2_autotrophic_dynamics/morphological_defenses.md) |
