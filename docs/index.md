...
* **Charnov MVT Foraging Kinetics & Softmax Selection:** Continuous spatial potential re-evaluation and Boltzmann stochastic action selection ($P(\text{move}) = \frac{\exp(F/\tau)}{\sum \exp(F/\tau)}$) allow swarms to leave depleting patches before plants hit zero resources.
